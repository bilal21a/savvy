        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title mb-0">
                    Categories
                    <a class="btn btn-sm btn-primary" style="float: right;" href="{{url("product-sub-categories/create")}}">Add new Category</a>
                </h4>
            </div>
            <div class="card-body">
...
<script>
    ...
    function loadCategories(){
        ...
        $('#categories').DataTable({
            ajax: '{{route("product.sub.categories.listing")}}',
            deferRender: true,
            order: [[0, 'desc']],
        });
    }
    function updateCategoryStatus(id,status){
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    type: "post",
                    url: "{{route('product.sub.categories.update.status')}}",
                    data: {
                        id : id,
                        status : status,
                        _token : "{{csrf_token()}}"
    ...
    function deleteCategory(id){
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    type: "delete",
                    url: "{{url('product-sub-categories')}}/"+id,
                    data: {
                        _token : "{{csrf_token()}}"
                    },
                    success: function (response) {
