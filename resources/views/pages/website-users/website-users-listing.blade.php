        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-title mb-0">
                    Users
                    {{-- <a class="btn btn-sm btn-primary" style="float: right;" href="{{url("website-users/create")}}">Add new User</a> --}}
                </h4>
            </div>
            <div class="card-body">
...
<script>
    ...
    function loadUsers(){
        ...
        $('#users').DataTable({
            ajax: '{{route("website.users.listing")}}',
            deferRender: true,
            order: [[0, 'desc']],
        });
    }
    function updateUserStatus(id,status){
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    type: "post",
                    url: "{{route('website.users.update.status')}}",
                    data: {
                        id : id,
                        status : status,
                        _token : "{{csrf_token()}}"
